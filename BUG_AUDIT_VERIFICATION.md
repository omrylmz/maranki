...
- **M9** (from the untested-17) — editing a card then pressing the **Android hardware-back** button raises a "Discard changes?" guard instead of silently dropping the edit. The `beforeRemove` interceptor works on the gesture that bypassed the on-screen back button. ✅
- **L6** (from the untested-17) — "Delete this card" raises a "Delete this card?" confirmation instead of deleting on first tap. ✅
- **Bonus (C1/M16):** M5's in-flight state survived an unrelated hard emulator (qemu) crash — the rated cards and streak persisted intact — extra evidence the persistence layer is robust to abrupt termination.

Not exercised on device (already covered by unit tests, and needing fault-injection / fixtures rather than UI taps): **C1** (corrupt blob), **H4** (malformed protobuf), **H5/H6** (CSV/`.apkg` import), plus **H2** (destructive to the test deck) and **H3** (no UI entry point on this build). The remaining untested-but-verified items are coverage debt, not defects.
